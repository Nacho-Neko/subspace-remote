//! Farming Remote controller
//!
//! Controller is responsible for managing farming Remote.
//!
//! This module exposes some data structures for NATS communication, custom piece getter and node
//! client implementations designed to work with Remote controller and a service function to drive
...
pub struct RemoteControllerFarmerIdentifyBroadcast;

impl GenericBroadcast for RemoteControllerFarmerIdentifyBroadcast {
...
#[derive(Debug, Copy, Clone, Encode, Decode)]
pub struct RemoteControllerCacheIdentifyBroadcast;

impl GenericBroadcast for RemoteControllerCacheIdentifyBroadcast {
...
struct RemoteControllerSlotInfoBroadcast {
    slot_info: SlotInfo,
    instance: String,
}

impl GenericBroadcast for RemoteControllerSlotInfoBroadcast {
...
#[derive(Debug, Clone, Encode, Decode)]
struct RemoteControllerRewardSigningBroadcast {
    reward_signing_info: RewardSigningInfo,
}

impl GenericBroadcast for RemoteControllerRewardSigningBroadcast {
    const SUBJECT: &'static str = "subspace.controller.reward-signing-info";
}

/// Broadcast with archived segment headers by controllers
#[derive(Debug, Clone, Encode, Decode)]
struct RemoteControllerArchivedSegmentHeaderBroadcast {
    archived_segment_header: SegmentHeader,
}

impl GenericBroadcast for RemoteControllerArchivedSegmentHeaderBroadcast {
...
#[derive(Debug, Clone, Encode, Decode)]
struct RemoteControllerSolutionNotification {
    solution_response: SolutionResponse,
}

impl GenericNotification for RemoteControllerSolutionNotification {
    const SUBJECT: &'static str = "subspace.controller.*.solution";
}

/// Notification messages with reward signature by farmers
#[derive(Debug, Clone, Encode, Decode)]
struct RemoteControllerRewardSignatureNotification {
    reward_signature: RewardSignatureResponse,
}

impl GenericNotification for RemoteControllerRewardSignatureNotification {
    const SUBJECT: &'static str = "subspace.controller.reward-signature";
}

/// Request farmer app info from controller
#[derive(Debug, Clone, Encode, Decode)]
struct RemoteControllerFarmerAppInfoRequest;

impl GenericRequest for RemoteControllerFarmerAppInfoRequest {
...
#[derive(Debug, Clone, Encode, Decode)]
struct RemoteControllerSegmentHeadersRequest {
    segment_indices: Vec<SegmentIndex>,
}

impl GenericRequest for RemoteControllerSegmentHeadersRequest {
...
#[derive(Debug, Clone, Encode, Decode)]
struct RemoteControllerFindPieceInCacheRequest {
    piece_index: PieceIndex,
}

impl GenericRequest for RemoteControllerFindPieceInCacheRequest {
...
#[derive(Debug, Clone, Encode, Decode)]
struct RemoteControllerFindPiecesInCacheRequest {
    piece_indices: Vec<PieceIndex>,
}

impl GenericStreamRequest for RemoteControllerFindPiecesInCacheRequest {
...
#[derive(Debug, Clone, Encode, Decode)]
struct RemoteControllerPieceRequest {
    piece_index: PieceIndex,
}

impl GenericRequest for RemoteControllerPieceRequest {
...
#[derive(Debug, Clone, Encode, Decode)]
struct RemoteControllerPiecesRequest {
    piece_indices: Vec<PieceIndex>,
}

impl GenericStreamRequest for RemoteControllerPiecesRequest {
...
/// Remote piece getter
#[derive(Debug, Clone)]
pub struct RemotePieceGetter {
    nats_client: NatsClient,
}

#[async_trait]
impl PieceGetter for RemotePieceGetter {
    async fn get_piece(&self, piece_index: PieceIndex) -> anyhow::Result<Option<Piece>> {
        if let Some((piece_cache_id, piece_cache_offset)) = self
            .nats_client
            .request(
                &RemoteControllerFindPieceInCacheRequest { piece_index },
                None,
            )
            .await?
        {
        ...
        Ok(self
            .nats_client
            .request(&RemoteControllerPieceRequest { piece_index }, None)
            .await?)
    }
        ...
        PieceIndices: IntoIterator<Item = PieceIndex, IntoIter: Send> + Send + 'a,
    {
...

        {
            let mut cached_pieces = self
                .nats_client
                .stream_request(
                    &RemoteControllerFindPiecesInCacheRequest { piece_indices },
                    None,
                )
                .await?;
...
/// [`NodeClient`] used in Remote environment that connects to node through a controller instead
...
impl NodeClient for RemoteNodeClient {
    async fn farmer_app_info(&self) -> anyhow::Result<FarmerAppInfo> {
        Ok(self
            .nats_client
            .request(&RemoteControllerFarmerAppInfoRequest, None)
            .await?
    ...
    async fn subscribe_slot_info(
        ...
        let subscription = self
            .nats_client
            .subscribe_to_broadcasts::<RemoteControllerSlotInfoBroadcast>(None, None)
            .await?
            .filter_map({
    ...
    async fn submit_solution_response(
        ...
        Ok(self
            .nats_client
            .notification(
                &RemoteControllerSolutionNotification { solution_response },
    ...
    async fn subscribe_reward_signing(
        ...
        let subscription = self
            .nats_client
            .subscribe_to_broadcasts::<RemoteControllerRewardSigningBroadcast>(None, None)
            .await?
    ...
    async fn submit_reward_signature(
        ...
        Ok(self
            .nats_client
            .notification(
                &RemoteControllerRewardSignatureNotification { reward_signature },
                None,
            )
            .await?)
    }
    ...
    async fn subscribe_archived_segment_headers(
        ...
        let subscription = self
            .nats_client
            .subscribe_to_broadcasts::<RemoteControllerArchivedSegmentHeaderBroadcast>(None, None)
            .await?
            .filter_map({
    ...
    async fn segment_headers(
        ...
        Ok(self
            .nats_client
            .request(
                &RemoteControllerSegmentHeadersRequest { segment_indices },
                None,
            )
            .await?)
    }
    ...
    NC: NodeClient,
{
    ...
    while let Some(slot_info) = slot_info_notifications.next().await {
        ...
        if let Err(error) = nats_client
            .broadcast(
                &RemoteControllerSlotInfoBroadcast {
                    slot_info,
    ...
    NC: NodeClient,
{
    ...
    while let Some(reward_signing_info) = reward_signing_notifications.next().await {
        ...
        if let Err(error) = nats_client
            .broadcast(
                &RemoteControllerRewardSigningBroadcast {
                    reward_signing_info,
                },
                instance,
            )
    ...
    NC: NodeClient,
{
    ...
    while let Some(archived_segment_header) = archived_segments_notifications.next().await {
        ...
        if let Err(error) = nats_client
            .broadcast(
                &RemoteControllerArchivedSegmentHeaderBroadcast {
                    archived_segment_header,
                },
                instance,
            )
    ...
    NC: NodeClient,
{
    let mut subscription = nats_client
        .subscribe_to_notifications::<RemoteControllerSolutionNotification>(
            Some(instance),
...
{
    let mut subscription = nats_client
        .subscribe_to_notifications::<RemoteControllerRewardSignatureNotification>(
            None,
            Some(instance.to_string()),
        )
        .await
    ...
    NC: NodeClient,
{
    nats_client
        .request_responder(
            ...
            |_: RemoteControllerFarmerAppInfoRequest| async move {
                Some(
                    node_client
                        .farmer_app_info()
                        .await
    ...
    NC: NodeClient,
{
    nats_client
        .request_responder(
            None,
            Some("subspace.controller".to_string()),
            |RemoteControllerSegmentHeadersRequest { segment_indices }| async move {
                node_client
...
async fn find_piece_responder(
    ...
    nats_client
        .request_responder(
            None,
            Some("subspace.controller".to_string()),
            |RemoteControllerFindPieceInCacheRequest { piece_index }| async move {
...
async fn find_pieces_responder(
    ...
    nats_client
        .stream_request_responder(
            None,
            Some("subspace.controller".to_string()),
            |RemoteControllerFindPiecesInCacheRequest { piece_indices }| async move {
    ...
    PG: PieceGetter + Sync,
{
    nats_client
        .request_responder(
            None,
            Some("subspace.controller".to_string()),
            |RemoteControllerPieceRequest { piece_index }| async move {
                piece_getter
    ...
    PG: PieceGetter + Sync,
{
    nats_client
        .stream_request_responder(
            None,
            Some("subspace.controller".to_string()),
            |RemoteControllerPiecesRequest { piece_indices }| async move {
                piece_getter
